...
def capture_old_order_status(sender, instance, **kwargs):
    try:
        ...
        logger.exception("Failed to capture old order status (pk=%s)", getattr(instance, "pk", None))
        instance._old_status = None
...
def capture_old_reservation_status(sender, instance, **kwargs):
    try:
        ...
        logger.exception("Failed to capture old reservation status (pk=%s)", getattr(instance, "pk", None))
        instance._old_status = None
...
def capture_old_inventory_count(sender, instance, **kwargs):
    try:
        ...
        logger.exception("Failed to capture old inventory count (pk=%s)", getattr(instance, "pk", None))
        instance._old_item_count = None
...
def notify_order_confirmed(sender, instance, created, **kwargs):
    try:
        ...
        logger.exception("Failed to notify order confirmed (order id=%s)", getattr(instance, "id", None))
...
def notify_reservation_confirmed(sender, instance, created, **kwargs):
    try:
        ...
        logger.exception(
            "Failed to notify reservation confirmed (reservation id=%s)", getattr(instance, "id", None)
        )
...
def notify_staff_low_inventory(sender, instance, created, **kwargs):
    try:
        ...
        logger.exception(
            "Failed to notify staff of low inventory (item=%s)", getattr(instance, "item_name", None)
        )
...
def notify_staff_new_review(sender, instance, created, **kwargs):
    try:
        ...
        logger.exception(
            "Failed to notify staff of new review (review id=%s)", getattr(instance, "id", None)
        )
